...
ensure_logfile(Name, FD, Inode, Buffer) ->
    case file:read_file_info(Name) of
        {ok, FInfo} ->
            ...
            case FD =/= undefined andalso Inode == Inode2 of
                true ->
                    {ok, {FD, Inode, FInfo#file_info.size}};
                false ->
...
get_rotate_logfile_name(File, Count) ->
    Suffix = ".log",
    NewFileName = case lists:suffix(Suffix, File) of
        true ->
            lists:concat([lists:sublist(File, length(File)-length(Suffix)), "_", Count, Suffix]);
        false ->
            lists:concat([File, ".", Count])
    end,
    NewFileName.
...
rotate_logfile(File, 1) ->
    NewFileName = get_rotate_logfile_name(File, 0),
    case file:rename(File, NewFileName) of
        ok ->
            ok;
        _ ->
            rotate_logfile(File, 0)
...
rotate_logfile(File, Count) ->
    _ = file:rename(get_rotate_logfile_name(File, Count - 2), get_rotate_logfile_name(File, Count - 1)),
...
parse_rotation_date_spec([$$, $S|Seconds]) ->
    case list_to_integer(Seconds) of
        X when X>0 ->
            {ok, [{second, X}]};
        _ ->
            {error, invalid_date_spec}
...
calculate_next_rotation([{second, Seconds}]) ->
    Seconds;
