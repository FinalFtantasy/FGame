...
do(State) ->
    ...
    case strip_flags(rebar_state:command_args(State)) of
        ...
        ["image", Input] ->
            TsungArgs = tsung_args(TsungConfig, #tsung_args{}),
            InputPath = os:cmd("cd "++ Input ++ " && pwd"),            
            OutputPath = string:trim(os:cmd("cd "++ TsungArgs#tsung_args.image_dir ++ " && pwd")),
            ImageCommand = "tsung_stats.pl --stats " ++ string:trim(InputPath) ++ "/tsung.log",
            %%io:format("image command ~p~n", [ImageCommand]),
            Timeout = proplists:get_value(timeout, TsungConfig, ?RUN_TSUNG_TIMEOUT),
            case run_tsung(ImageCommand, Timeout, OutputPath) of
                {ok, _Res} ->
                    {ok, State};
                Error ->
                    io:format("2"),
        ...
        ["func", Func] -> 
            ...
            Command = parse_command2(TsungArgs, Func),
...
tsung_args([{default_config, Config} | Rest], #tsung_args{} = Args) ->
    tsung_args(Rest, Args#tsung_args{default_config = Config});
...
tsung_args([{image_dir, ImageDir} | Rest], #tsung_args{} = Args) ->
    tsung_args(Rest, Args#tsung_args{image_dir = ImageDir});
...
tsung_args([{include_dirs, IncludePA} | Rest], #tsung_args{include_pa = OldIncludePA} = Args) ->
    tsung_args(Rest, Args#tsung_args{include_pa = IncludePA ++ OldIncludePA});
...
parse_command2(#tsung_args{root = Root} = Args, Func) ->
    ParamsTsung = [?TSUNG_COMMAND],

    {ParamsConfig, ConfigFile} =
        case Args#tsung_args.default_config of
            undefined ->
                {[filename:join(Root, "tsung.xml"), "-f" | ParamsTsung], filename:join(Root, "tsung.xml")};
            Config ->
                {[Config, "-f" | ParamsTsung], Config}
        end,

    SedCommand = "sed -i \"s/%%.*%%/%%" ++ Func ++ "%%/g\" "++ ConfigFile,
    os:cmd(SedCommand),
...
run_tsung(Command, Timeout, Dir) ->
    io:format("Tsung run command:~p~n", [Command]),
    Port = erlang:open_port({spawn, Command}, [exit_status, {cd, Dir}]),
    tsung_loop(Port, [], Timeout).
